...
class User:
    ...
    def validate_user(user): 
        ...
        if len(user['fname']) < 1: 
            ...
            flash('first name must be a minimum than two characters', 'regError')
            is_valid = False
        elif not NAME_REGEX.match(user['fname']): 
            flash("First name cannot contain numbers or spaces(unless you're Elon Musks child)!", 'regError')
            is_valid = False
        if len(user['lname']) < 1:
            flash("must enter a last name", 'regError')
            is_valid = False
        elif len(user['lname']) < 2:
            flash('last name must be a minimum than two characters', 'regError')
            is_valid = False
        elif not NAME_REGEX.match(user['lname']): 
            flash("Last name cannot contain numbers or spaces (unless you're Elon Musks child)!", 'regError')
            is_valid = False
        if len(user['email']) < 1: 
