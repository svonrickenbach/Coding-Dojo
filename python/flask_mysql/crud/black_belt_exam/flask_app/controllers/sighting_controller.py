...
def show_sighting(sighting_id): 
    if 'user_id' not in session: 
        return redirect('/')
    return render_template('view_sighting.html', current_user = user.User.getByID({'id' : session['user_id']}), sightings = sighting.Sighting.getByID_w_user({'sighting_id' : sighting_id}), dtf = dateFormat_full_month, skeptic = skeptic.Skeptic.getByID({'user_id' : session['user_id']}))
